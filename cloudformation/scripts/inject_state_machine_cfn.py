...
def usage():
    ...
    print('%s -s <state-machine-file> -c <cfn-file>' % arg)
...
def main(argv):
    ...
    parser.add_argument(
        '-s', '--state-machine-file',
        required=True, dest='state_machine'
    )
    parser.add_argument(
        '-c', '--cfn-file',
        required=True, dest='cfn_template'
    )
    parser.add_argument(
        '-o', '--output-file',
        dest='output', default='complete.yaml'
    )
